...
const nodeTypes = [
  ...
  { type: 'approval', label: '审批节点', icon: 'https://cdn.jsdelivr.net/gh/LogicFlow/static@latest/docs/static/approve.png' },
  { type: 'condition', label: '条件分支', icon: 'https://cdn.jsdelivr.net/gh/LogicFlow/static@latest/docs/static/condition.png' },
  ...
  return (
    <div className={styles.designerContainer}>
      ...
      <div style={{ marginTop: 16, display: 'flex', gap: 12 }}>
        <button onClick={() => onSave?.(getGraphData())} style={{ padding: '8px 16px', backgroundColor: '#1677ff', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>保存流程</button>
        <button onClick={() => onDeploy?.()} style={{ padding: '8px 16px', backgroundColor: '#52c41a', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>部署流程</button>
      </div>
    </div>
  );
};
