...
import { Card, Input, Button, List, Avatar, Spin, message, Empty, Tooltip, Tag, Modal } from 'antd'
import {
  SendOutlined, ClearOutlined, RobotOutlined, UserOutlined,
  LoadingOutlined, CopyOutlined, CheckCircleFilled, ExclamationCircleFilled,
  CommentOutlined, ClockCircleOutlined
          ...
          <div style={{
            ...
            <ReactMarkdown
              ...
              components={{
                ...
                strong({ children }) { return <strong style={{ color: '#1890ff' }}>{children}</strong> },
                h1({ children }) { return <h1 style={{ fontSize: 18, margin: '8px 0' }}>{children}</h1> },
                h2({ children }) { return <h2 style={{ fontSize: 16, margin: '8px 0' }}>{children}</h2> },
                h3({ children }) { return <h3 style={{ fontSize: 14, margin: '6px 0' }}>{children}</h3> },
                blockquote({ children }) {
...
const AgentSharePage = () => {
  ...
  const verifyAndLoad = async () => {
    ...
    try {
      ...
      if (res.data) {
        ...
        setAppName(res.data.appName || 'Agent')
        setAppDesc(res.data.appDesc || '')
        setConnected(true)
        // 加载历史消息
        try {
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (res.data) {
        assistantContent = res.data.assistantMessage || '好的，我明白了。'
        setMessages(prev => prev.map(m =>
          m.id === assistantMsgId
            ? { ...m, content: assistantContent, status: 'SUCCESS', latencyMs: res.data.latencyMs }
            : m
        ))
      } else {
