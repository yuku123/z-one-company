...
import { Card, Tabs, Table, Button, Space, Modal, Form, Input, Select, message, Popconfirm, Tag, Descriptions, InputNumber, Typography } from 'antd'
import { PlusOutlined, DeleteOutlined, ReloadOutlined, ApiOutlined, PlayCircleOutlined, LinkOutlined } from '@ant-design/icons'
...
export default function McpPage() {
  ...
  const handleSelectTool = (tool) => {
    ...
    if (schema?.properties) {
      const defaults = {}
      Object.keys(schema.properties).forEach(k => { defaults[k] = '' })
  ...
  return (
    <div style={{ height: 'calc(100vh - 180px)', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: selectedServer ? '0 0 50%' : 1, overflow: 'auto' }}>
            <Table columns={columns} dataSource={servers} rowKey="id" loading={loading}
              ...
              onRow={(r) => ({
                ...
                style: { background: selectedServer?.id === r.id ? '#e6f7ff' : undefined, cursor: 'pointer' }
              })} />
          </div>

          {/* Tool Explorer */}
          {selectedServer && (
            <div style={{ flex: '0 0 50%', borderLeft: '1px solid #f0f0f0', paddingLeft: 16, overflow: 'auto' }}>
              <div style={{ marginBottom: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text strong style={{ fontSize: 15 }}>{selectedServer.serverName} — 工具列表</Text>
                <Button size="small" icon={<ReloadOutlined />} onClick={() => handleSelectServer(selectedServer)}>刷新</Button>
              </div>
              {tools.length === 0 ? (
                ...
                tools.map(t => (
                  <Card key={t.name} size="small" hoverable
                    style={{ marginBottom: 8, borderColor: selectedTool?.name === t.name ? '#1890ff' : undefined }}
                    onClick={() => { handleSelectTool(t); setActiveTab('test') }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <Tag color="blue" style={{ cursor: 'pointer' }}
                        onClick={(e) => { e.stopPropagation(); handleSelectTool(t); setActiveTab('test') }}>
                        测试 →
                      </Tag>
                    </div>
                  </Card>
      ...
      {activeTab === 'test' && selectedTool && (
        <div style={{ flex: 1, display: 'flex', gap: 16, overflow: 'hidden' }}>
          ...
          <div style={{ flex: '0 0 60%', display: 'flex', flexDirection: 'column' }}>
            ...
            <pre style={{ flex: 1, overflow: 'auto', background: '#f5f5f5', padding: 12, borderRadius: 4,
        ...
        {testResult?.loading ? <Text>测试中...</Text> : testResult ? (
          <Descriptions column={1} size="small">
            <Descriptions.Item label="连接状态">
              <Tag color={testResult.connected ? 'green' : 'red'}>{testResult.connected ? '成功' : '失败'}</Tag>
            </Descriptions.Item>
            {testResult.serverInfo && <Descriptions.Item label="服务信息">
              {testResult.serverInfo?.name} v{testResult.serverInfo?.version}
            </Descriptions.Item>}
            {testResult.toolCount !== undefined && <Descriptions.Item label="工具数量">{testResult.toolCount}</Descriptions.Item>}
      ...
      <Modal title={editing ? '编辑 MCP 服务' : '添加 MCP 服务'} open={modalOpen}
        ...
        <Form form={form} layout="vertical" initialValues={{ transportType: 'HTTP', timeout: 60, status: 'active' }}>
