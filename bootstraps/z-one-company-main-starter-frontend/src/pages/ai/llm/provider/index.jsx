...
import { Table, Button, Space, Modal, Form, Input, Select, message, Tag, Divider, Row, Col, Card, Statistic } from 'antd'
...
const LlmProvider = () => {
  ...
  const handleDelete = async (record) => {
    confirm({
      ...
      onOk: async () => {
        await request('/api/llm-center/provider/delete', { method: 'POST', data: { id: record.id } })
  ...
  const handleSubmit = async () => {
    try {
      ...
      await request(editing ? '/api/llm-center/provider/update' : '/api/llm-center/provider/create', {
        method: 'POST',
        data: values
      })
  ...
  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}>
          <Card><Statistic title="活跃模型" value={data.reduce((sum, p) => sum + (p.modelCount || 0), 0)} /></Card>
        </Col>
      </Row>
