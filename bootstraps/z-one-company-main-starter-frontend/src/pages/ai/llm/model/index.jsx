...
import { Table, Button, Space, Modal, Form, Input, InputNumber, Select, message, Tag, Row, Col, Card, Statistic } from 'antd'
...
const LlmModel = () => {
  ...
  const fetchModels = async () => {
    setLoading(true)
    try {
      const res = await request('/api/llm-center/model/list', { method: 'GET' })
      setData(res.data || [])
    } catch (e) {
      message.error('加载失败')
    } finally {
  ...
  const fetchProviders = async () => {
    try {
      const res = await request('/api/llm-center/provider/list', { method: 'GET' })
  ...
  const handleDelete = async (record) => {
    confirm({
      ...
      onOk: async () => {
        await request('/api/llm-center/model/delete', { method: 'POST', data: { id: record.id } })
  ...
  const handleSubmit = async () => {
    try {
      ...
      await request(editing ? '/api/llm-center/model/update' : '/api/llm-center/model/create', {
        method: 'POST',
        data: values
      })
  ...
  return (
    <div>
      <Row gutter={16} style={{ marginBottom: 16 }}>
        ...
        <Col span={6}>
          <Card><Statistic title="总上下文" value={data.reduce((sum, m) => sum + (m.contextWindow || 0), 0).toLocaleString()} suffix="tokens" /></Card>
        </Col>
      </Row>
        ...
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="providerCode" label="供应商" rules={[{ required: true }]}>
            <Select placeholder="选择供应商">
              {providers.map(p => (
                <Select.Option key={p.providerCode} value={p.providerCode}>{p.providerName}</Select.Option>
              ))}
            </Select>
          </Form.Item>
          <Row gutter={16}>
