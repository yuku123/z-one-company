...
const ClusterPage = () => {
  ...
  const fetchList = async () => {
    ...
    } catch (e) { message.error('获取集群列表失败') } finally { setLoading(false) }
  }
  ...
  const handleSave = async (values) => {
    try {
      ...
      setModalOpen(false); setEditingRecord(null); form.resetFields(); fetchList()
      ...
      render: (_, record) => (
        <Space>
          <Button type="link" size="small" icon={<EditOutlined />}
            onClick={() => { setEditingRecord(record); form.setFieldsValue(record); setModalOpen(true) }}>编辑</Button>
    ...
    <div>
      <Card title="命名空间管理"
        extra={<Space><Button icon={<ReloadOutlined />} onClick={fetchList} loading={loading}>刷新</Button>
          <Button type="primary" icon={<PlusOutlined />} onClick={() => { setEditingRecord(null); form.resetFields(); setModalOpen(true) }}>新建</Button></Space>}>
