...
import { Card, Table, Tag, Button, Space, message, Descriptions, Badge, Tooltip } from 'antd'
import { ArrowLeftOutlined, ReloadOutlined, CheckCircleOutlined, CloseCircleOutlined, QuestionCircleOutlined } from '@ant-design/icons'
...
const ServiceDetail = () => {
  ...
  useEffect(() => { if (fullName) fetchInstances() }, [fullName])
  ...
  const columns = [
    ...
    { title: '元数据', dataIndex: 'metadata', key: 'metadata', ellipsis: true,
      render: (m) => {
        ...
        return <Tooltip title={<pre style={{ margin: 0, fontSize: 12 }}>{JSON.stringify(typeof m === 'object' ? m : JSON.parse(m), null, 2)}</pre>}><span style={{ cursor: 'pointer' }}>{str}</span></Tooltip>
      }},
  ]
    ...
    <div>
      ...
      <Card size="small" style={{ marginBottom: 16 }}>
        <Descriptions size="small" column={4}>
          <Descriptions.Item label="总实例"><span style={{ fontSize: 20, fontWeight: 600 }}>{instances.length}</span></Descriptions.Item>
          ...
          <Descriptions.Item label="异常">
            <span style={{ color: unhealthyCount > 0 ? '#ff4d4f' : undefined, fontSize: 20, fontWeight: 600 }}>{unhealthyCount}</span>
