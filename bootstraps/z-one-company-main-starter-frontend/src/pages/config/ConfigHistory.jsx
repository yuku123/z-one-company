...
const ConfigHistory = () => {
  ...
  useEffect(() => { fetchHistoryList(); loadFilterOptions() }, [])

  const loadFilterOptions = async () => {
    try { const r = await configApi.clusterList(); setNamespaceList((r||[]).map(i=>({label:i.name,value:i.name}))) } catch(e){}
    try { const r = await configApi.groupList(); setGroupList((r||[]).map(i=>({label:i,value:i}))) } catch(e){}
  }
      ...
      <div style={{ marginBottom: 16 }}>
        <Space size="middle">
          ...
          <Input.Search placeholder="搜索 Data ID..." value={searchText}
            onChange={e=>setSearchText(e.target.value)} onSearch={()=>fetchHistoryList(1,pagination.pageSize)}
