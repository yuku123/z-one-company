...
import { ReloadOutlined, EyeOutlined, CloudServerOutlined } from '@ant-design/icons'
...
const ServiceList = () => {
  ...
  const columns = [
    { title: '服务名', dataIndex: 'serviceName', key: 'serviceName',
      render: (v) => <Space><CloudServerOutlined style={{ color: '#1677ff' }} /><span style={{ fontWeight: 500 }}>{v}</span></Space> },
    ...
    { title: '集群数', dataIndex: 'clusterMap', key: 'clusterCount', width: 80,
      render: (v) => {
        try { return Object.keys(JSON.parse(v || '{}')).length } catch { return 0 }
      }},
    ...
    { title: '健康状态', key: 'health', width: 100,
      render: (_, r) => {
        try {
          ...
          return allHealthy ? <Badge status="success" text="健康" /> : hasUnhealthy ? <Badge status="error" text="异常" /> : <Badge status="default" text="未知" />
        } catch { return <Badge status="default" text="未知" /> }
      }},
