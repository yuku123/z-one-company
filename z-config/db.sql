-- 创建Nacos数据库（若需单独指定）
CREATE DATABASE IF NOT EXISTS z_config CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
USE z_config;
...
CREATE TABLE IF NOT EXISTS `z_config_info` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_z_config_info_data_group_namespace` (`data_id`,`group`,`namespace`)
...
CREATE TABLE IF NOT EXISTS `z_config_info_beta` (
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_z_configinfobeta_datagroupnamespace` (`data_id`,`group`,`namespace`)
...
CREATE TABLE IF NOT EXISTS `z_config_info_history` (
    ...
    `namespace` varchar(128) DEFAULT '' COMMENT '命名空间',
    PRIMARY KEY (`id`),
    KEY `idx_z_gmt_modified` (`gmt_modified`),
    KEY `idx_z_did` (`data_id`,`group`,`namespace`)
...
CREATE TABLE IF NOT EXISTS `z_service_info` (
    ...
    `gmt_modified` datetime NOT NULL COMMENT '修改时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_z_serviceinfo_servicename_namespaceid` (`service_name`,`namespace`)
...
CREATE TABLE IF NOT EXISTS `z_instance` (
    `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `service_id` bigint(20) NOT NULL COMMENT '关联z_service_info.id',
    ...
    `gmt_modified` datetime NOT NULL COMMENT '修改时间',
    PRIMARY KEY (`id`),
    KEY `idx_z_service_id` (`service_id`),
    KEY `idx_z_ip_port` (`ip`,`port`),
    KEY `uk_z_instanceid` (`instance_id`)
...
CREATE TABLE IF NOT EXISTS `z_cluster` (
    ...
    `gmt_modified` datetime NOT NULL COMMENT '修改时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_z_cluster_serviceid_name` (`service_id`,`name`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='集群表';

CREATE TABLE IF NOT EXISTS `z_subscription` (
    `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
    `consumer_instance_id` varchar(255) NOT NULL COMMENT '消费实例ID（同instance_id格式）',
    `consumer_ip` varchar(64) NOT NULL COMMENT '消费实例IP',
    `consumer_port` int(11) NOT NULL COMMENT '消费实例端口',
    `subscribe_service_id` bigint(20) NOT NULL COMMENT '订阅的服务ID（关联z_service_info.id）',
    ...
    KEY `idx_subscribe_serviceid` (`subscribe_service_id`)
...
CREATE TABLE IF NOT EXISTS `z_permissions` (
                                               `role` varchar(50) NOT NULL COMMENT '关联z_roles.role',
    `resource` varchar(255) NOT NULL COMMENT '资源标识（如配置data_id:group）',
