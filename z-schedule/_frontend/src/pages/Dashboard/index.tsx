...
import { Row, Col, Card, Statistic, Table, Tag, Timeline, Progress, List, Avatar, Badge } from 'antd';
import {
  DashboardOutlined,
  ScheduleOutlined,
  FileTextOutlined,
...
import { DashboardStats, ScheduleRecord } from '@/types';
    ...
    pieChart.setOption({
      ...
      series: [{
        ...
        data: [
          ...
          { value: (stats?.jobCount || 0) - (stats?.runningJobCount || 0), name: '已停止', itemStyle: { color: '#ff4d4f' } },
        ],
      }],
    });
          ...
          <Card title="执行器负载状态" loading={loading}>
            <List
              dataSource={[
                ...
                { appName: 'notification-service', instanceCount: 1, runningTasks: 3, cpu: 15, memory: 30 },
              ]}
              renderItem={(item) => (
                <List.Item>
                  <div style={{ width: '100%' }}>
                    ...
                    <div style={{ display: 'flex', gap: 24 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <Progress percent={item.cpu} size="small" strokeColor="#1677ff" showInfo={false} />
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 12, color: '#8c8c8c', marginBottom: 4 }}>
                          内存: {item.memory}%
                        </div>
                        <Progress percent={item.memory} size="small" strokeColor="#52c41a" showInfo={false} />
                      </div>
                    </div>
                  </div>
                </List.Item>
