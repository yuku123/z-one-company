...
| 序号 | 事项 | 优先级 |
|------|------|--------|
| F1 | **登录页** `/ctc/login` — 用户名密码登录、手机验证码登录、第三方登录入口 | P0 |
| F2 | **注册页** `/ctc/register` — 用户名/手机/邮箱注册 | P0 |
| F3 | **重置密码页** — 手机/邮箱验证码重置 | P0 |
| F4 | **概览页** `/ctc/overview` — 当前用户信息、快捷操作 | P0 |
| F5 | **账号管理** `/ctc/admin/account` — 用户列表、新增、编辑、分配角色 | P1 |
| F6 | **角色管理** `/ctc/admin/role` — 角色列表、新增、编辑、分配权限 | P1 |
| F7 | **权限管理** `/ctc/admin/permission` — 权限树形结构、新增、编辑、排序 | P1 |
| F8 | **租户管理** `/ctc/admin/tenant` — 租户列表、新增、编辑（超管） | P1 |
| F9 | **审计日志** `/ctc/admin/audit` — 操作日志查询、筛选、导出 | P2 |
| F10 | **域/组织/部门管理** `/ctc/admin/org` — 层级结构管理 | P2 |
...
与主前端一致：React + Umi Max + Ant Design + Vite

- 路由前缀：`/ctc/*`
- 状态管理：React Context 或 Umi 的 useModel
...
| F1 | 登录页（用户名密码 + 手机验证码 + 第三方登录入口） | 2 |
| F2 | 注册页（用户名/手机/邮箱） | 1 |
| F3 | 重置密码页 | 0.5 |
| F4 | 概览页（当前用户信息） | 0.5 |
| | **Phase 1 小计** | **12 人天** |

### Phase 2：账号权限管理（1.5 周）
...
| F5 | 账号管理页（用户 CRUD + 分配角色） | 2 |
| F6 | 角色管理页（角色 CRUD + 分配权限） | 2 |
| F7 | 权限管理页（树形结构 + 排序 + 图标） | 2 |
| | **Phase 2 小计** | **9 人天** |

### Phase 3：租户域管理 + SSO（1 周）

| 序号 | 事项 | 人天 |
|------|------|------|
| P1-4 | 租户隔离拦截器（MyBatis 拦截器统一注入 tenant_id） | 2 |
| P1-5 | SSO Auth Server 端点（/auth/verify）实现 | 0.5 |
| P1-6 | Domain → Org → Dept → Group 层级查询（带 domain_id 传递） | 1 |
| F8 | 租户管理页（新增/编辑，超管权限） | 1 |
| F9 | 域/组织/部门管理页（树形结构） | 2 |
| | **Phase 3 小计** | **6.5 人天** |
...
| 序号 | 事项 | 人天 |
|------|------|------|
| E1 | 审计日志查询页（筛选/分页/导出） | 1 |
| E2 | 操作审计日志详情（请求参数/响应/耗时） | 0.5 |
| E3 | 密码强度校验 | 0.5 |
| E4 | 账号锁定（连续输错 N 次密码锁定） | 0.5 |
| E5 | 文档输出（ Knife4j + 各接口说明） | 0.5 |
| | **Phase 4 小计** | **3 人天** |
...
| 阶段 | 内容 | 人天 |
|------|------|------|
| Phase 1 | 数据层修复 + 认证闭环 + 登录注册前端 | 12 |
| Phase 2 | 账号权限管理（前后端） | 9 |
| Phase 3 | 租户域管理 + SSO + 前端 | 6.5 |
| Phase 4 | 收尾（审计日志/安全增强/文档） | 3 |
| **总计** | | **30.5 人天 ≈ 6 周** |

> 注：以上为单模块独立工期估算，与其他模块并行工作互不影响。
